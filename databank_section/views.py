...
def send_matching_pdf(request, property_id):
    try:
        ...
        logo = RLImage(logo_path, width=50, height=50)  # Resize as needed
        ...
        header_data = [[
            logo,
            Paragraph("<b>DEVELOK DEVELOPERS</b><br/>Thrissur, Kerala<br/>📞 9846845777 | 9645129777<br/>✉️ info@devlokdevelopers.com | 🌐 www.devlokdevelopers.com", header_sub_style)
        ]]
        ...
        for score, prop in ranked_matches[:5]:
            ...
            for img_obj in prop.images.all()[:2]:
                try:
                    img = RLImage(img_obj.image.path, width=3*inch, height=2*inch)
                    img.hAlign = 'LEFT'
                    content.append(img)
                    content.append(Spacer(1, 6))
